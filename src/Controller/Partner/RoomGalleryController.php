...
class RoomGalleryController extends AbstractController
{
    ...
    #[Route(path: '/rooms/{room_id}/gallery', name: 'app_partner_room_gallery_index', requirements: ['room_id' => '\d+'])]
    public function index(Request $request, $room_id)
    {
        ...
        return $this->render('partner/roomGallery/index.html.twig', [
            'galleries' => $galleries,
            'room' => $room,
        ]);
    }

    #[Route(path: '/rooms/{room_id}/gallery/add', name: 'app_partner_room_gallery_add', requirements: ['room_id' => '\d+'], options: ['expose' => true])]
    public function add(Request $request, $room_id)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('app_partner_room_gallery_index', ['room_id' => $room->getId()]);
        }

        return $this->render('partner/roomGallery/add.html.twig', [
            'form' => $form->createView(),
            'room' => $room,
        ]);
    }

    #[Route(path: '/rooms/{room_id}/gallery/{id}/move', name: 'app_partner_room_gallery_move', requirements: ['id' => '\d+', 'room_id' => '\d+'])]
    ...
    #[Route(path: '/room-gallery/{id}/delete', name: 'app_partner_room_gallery_delete', requirements: ['id' => '\d+'], options: ['expose' => true])]
    ...
    #[Route(path: '/room-gallery/bulk/delete', name: 'app_partner_room_gallery_bulk_delete', options: ['expose' => true])]
    ...
    private function deleteForm(RoomGallery $gallery)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('app_partner_room_gallery_delete', ['id' => $gallery->getId()]))
            ->getForm();
    }
    ...
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('app_partner_room_gallery_bulk_delete'))
            ->getForm();
    }

    private function accessDeniedException(RoomGallery $gallery)
    {
        if ($gallery->getRoom()->getHostel()->getOwner() !== $this->getUserOrThrow()) {
            throw $this->createAccessDeniedException();
        }
    }
